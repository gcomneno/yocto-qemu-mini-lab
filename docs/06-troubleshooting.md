...
Check which build directory is large.
Keep downloads if possible.
Keep sstate-cache if possible.
Delete build/tmp only when needed.
Source the build environment again.
Rerun the BitBake command.
